# agent7.py — Person B's deliverable ("Claude Brain"), Gemini edition
import os
import json
import time
...
RISK_METADATA = {
    "book_flight": {"risk": "Medium", "reversible": "No — refund only, with fees"},
    "book_hotel": {"risk": "Medium", "reversible": "No — refund/cancellation policy dependent"},
    "make_payment": {"risk": "High", "reversible": "No — irreversible once processed"},
}
...
class BoundaryAgent:
        ...
        for attempt in range(6):
            try:
                ...
                if "429" in err_msg or "RESOURCE_EXHAUSTED" in err_msg or "TOO MANY REQUESTS" in err_msg:
                    ...
                    continue
                raise e
        raise Exception("API rate limit retry limit exceeded. Please wait a moment.")
    ...
    def _process_loop(self, response) -> dict:
        while True:
            ...
            self.history.append(types.Content(role="model", parts=[
                types.Part(function_call=fc) for fc in response.function_calls
            ]))

            # Gemini can return more than one function call in a single turn.
            # Every function call in this model turn MUST get a matching
            # function response in the next turn, in order, or the next
            # generate_content call is a malformed request (400 ClientError).
            tool_responses = []
            for tool_call in response.function_calls:
                ...
                if needs_approval(tool_name):
                    # Stash any already-executed responses from this same
                    # turn so they aren't lost, then pause for approval.
                    if tool_responses:
    ...
    def _execute_tool(self, name: str, args: dict):
        ...
        try:
            ...
            self.action_log.append({
                "tool": name, "arguments": args, "status": "Success", "output": str(output)
            })
            return output
        except Exception as e:
            self.action_log.append({
                "tool": name, "arguments": args, "status": "Failed", "output": str(e)
            })
    ...
    def resolve_approval(self, approved: bool) -> dict:
        ...
        if approved:
            ...
            self.history.append(types.Content(role="user", parts=[
                types.Part.from_function_response(name=tool_name, response={"result": result_data})
            ]))
            response = self._send_with_retry()
            return self._process_loop(response)
        else:
            self.action_log.append({
                "tool": tool_name, "arguments": args, "status": "Denied", "output": "Action cancelled by user."
            })
            self.history.append(types.Content(role="user", parts=[
                types.Part.from_function_response(name=tool_name, response={"result": "Error: Action rejected by user authorization rules."})
            ]))
            response = self._send_with_retry()
            return self._process_loop(response)
