...
- search_flights uses Duffel (live) if DUFFEL_API_KEY is set, otherwise \
mock data. If the user specifically wants to see real live flight \
offers, or asks to check flights again after making some other real \
booking, call search_real_flights directly instead — same live Duffel \
data, but it never falls back to mock data, so use it when the user \
needs to know for sure the results are real.
...
TOOL_FUNCTIONS = {
    ...
    "search_real_flights": tools.search_real_flights,
...
TOOLS = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "search_real_flights",
            "description": "Look up REAL flight offers between two airports using live data from Duffel (real airlines, times, and prices). Returns a bookable flight_id — same data source as search_flights when Duffel is active, exposed here so it can be called directly.",
            "parameters": {
                "type": "object",
                "properties": {
                    "origin": {"type": "string", "description": "3-letter airport code, e.g. BLR"},
                    "destination": {"type": "string", "description": "3-letter airport code, e.g. DEL"},
                    "date": {"type": "string", "description": "YYYY-MM-DD, optional — defaults to ~2 weeks out"},
                },
                "required": ["origin", "destination"],
            },
        },
