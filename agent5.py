"""
agent4.py — Person B's deliverable ("Claude Brain"), Gemini edition
...
class BoundaryAgent:
    ...
    def _send_with_retry(self, content):
        """Helper to send messages to Gemini with pacing and 429 backoff."""
        time.sleep(4.5)  # Safe request spacing delay
        ...
        for attempt in range(5):
            try:
                ...
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e).upper():
                    ...
                    continue
                raise e
        raise Exception("API rate limit retry limit exceeded.")
    ...
    def _process_loop(self, response) -> dict:
        while True:
            ...
            # Extract out the tool call from the function_calls iterable object safely
            tool_call = response.function_calls[0] if isinstance(response.function_calls, list) else response.function_calls
